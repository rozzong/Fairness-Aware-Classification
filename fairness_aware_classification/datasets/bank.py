...
class BankDataset:
    ...
    def __init__(self):
        ...
        file_exists = os.path.isfile(DATA_PATH + os.sep + \
            "bank_additional_full_preprocessed.csv")
            
        if not file_exists:
            # Load and split the data
            file_path = RAW_DATA_PATH + os.sep + "bank_additional_full.csv"
            df = pd.read_csv(file_path, sep=';')
            # ~ file_path = DATA_PATH + os.sep + "bank_additional_full_processed.csv"
            ...
            # Define a new feature in the case a cient was not contacted
            df = df.assign(contacted=pd.Series(["yes"]*len(df)).values)
            ...
            # Save the dataset
            df.to_csv(
                DATA_PATH + os.sep + "bank_additional_full_preprocessed.csv",
            )
        
        else:
            # Load and split the data
            file_path = DATA_PATH + os.sep + "bank_additional_full_preprocessed.csv"
            df = pd.read_csv(file_path, index_col=0)
