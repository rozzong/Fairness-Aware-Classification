...
class COMPASDataset:
    ...
    def __init__(self):
        ...
        file_exists = os.path.isfile(DATA_PATH + os.sep + \
            "compas-scores-two-years_preprocessed.csv")
            
        if not file_exists:
            # Load and split the data
            file_path = RAW_DATA_PATH + os.sep + "compas-scores-two-years.csv"
            df = pd.read_csv(file_path, index_col=0)
            
            # Keep only features of interest and the target
            df = df[["sex", "age_cat", "race", "priors_count", "c_charge_degree", "two_year_recid"]]
            
            # Force the target to be categorical
            df = df.astype({"two_year_recid": object})
            ...
            # Save the dataframe
            df.to_csv(
                DATA_PATH + os.sep + "compas-scores-two-years_preprocessed.csv",
            )
                    
        else:
            # Load and split the data
            file_path = DATA_PATH + os.sep + "compas-scores-two-years_preprocessed.csv"
            df = pd.read_csv(file_path, index_col=0)
        
        # Set the target and do some feature selection
        self.y = df.pop("two_year_recid")
